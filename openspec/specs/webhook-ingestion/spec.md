...
contenido no vacío y con `account_id` + `conversation_id` presentes. Cualquier otro
evento se ignora sin error.

#### Scenario: Evento no procesable
- **WHEN** el evento no es `message_created`, o el `message_type` no es `incoming`, o el contenido está vacío
...
#### Scenario: Mensaje entrante válido
- **WHEN** el evento es `message_created`, `incoming`, con contenido e identificadores
