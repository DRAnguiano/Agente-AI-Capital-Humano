...
- [x] 5a.1 RED+impl rama **escuelita pre-handoff**: preguntar si tiene licencia B/E vigente o comprobante de cita; si sí → canalizar con esa info; si no → indicar que debe tener licencia vigente para ser considerado y no continuar funnel
- [x] 5a.2 RED+impl rama **CECATI pre-handoff**: mismo flujo de licencia que 5a.1 (sin licencia vigente/trámite no aplica aún; con licencia → revisar escuelita disponible)
- [x] 5a.3 RED+impl rama **B1 pre-handoff**: preguntar tipo de unidad (full/sencillo) y si tiene licencia y apto vigentes antes de canalizar; nota IA refleja lo recolectado
- [x] 5a.4 RED+impl rama **reingreso pre-handoff**: preguntar tipo de vacante (operador u otra); si operador → verificar ciudad + licencia + apto; si otra vacante → canalizar directo sin funnel adicional; nota IA refleja motivo
- [x] 5a.5 Verificar que `Siguiente acción` en nota IA para reingreso NO diga "continuar flujo" sino la acción concreta pendiente (verificar historial, confirmar vacante, etc.)

## 6. Verificación y cierre

- [x] 6.1 Suite Groq-free en `api-test` verde (actualizar tests del formato técnico viejo, no ignorarlos)
- [x] 6.2 Rebuild + recreate; verificación 1×1 en producción (chat real) por rama
- [x] 6.3 `openspec validate funnel-and-note-redesign --strict` + `openspec validate --specs --strict`
