...
- [x] 10b.17 **Follow-up solo por canal productivo Chatwoot** — contrato en
  `openspec/specs/followup-scheduler/spec.md`. **Implementado** en `app/followup/scheduler.py` ·
  `is_eligible_for_followup(lead_key)` (helper puro; el canal sale del prefijo del `lead_key`
  "{channel}:{id}", sin tocar la vista SQL): `chatwoot` productivo; `telegram_demo` solo con
  `ENABLE_DEMO_FOLLOWUP=true`; prefijos `test_*`/`debug_*`/`shadow_test*` bloqueados; canal directo
  desconocido (whatsapp/web sin Chatwoot) no productivo. Aplicado a las 2 queries de `run_scheduler`
  (seguimiento normal + solicitud_llamada) → omitidas con motivo `canal_no_productivo`.
  Tests: `tests/test_followup_scheduler_channels.py` (8 deterministas).
