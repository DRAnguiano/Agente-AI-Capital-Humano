...
- [x] 2.1 RED: matriz de la tricotomía experiencia-objetivo (objetivo_full_sencillo / considerar_escuelita_transmontes / cecati_sugerido) con exclusividad mutua y precedencia objetivo > escuelita > cecati
- [x] 2.2 Implementar la resolución con precedencia en `app/chatwoot_note_sync.py` · `calculate_candidate_labels`
- [x] 2.3 RED+impl: `aclaracion_pendiente` cuando `experience.vehicle_type_pending` y unidad no confirmada; se retira al confirmar full/sencillo
- [x] 2.4 RED+impl: `considerar_operador_b1` desde `experience.b1_us_intent` → canaliza a humano (marca `requiere_agente`/revisión), no bloquea el resto del perfil
- [x] 2.5 RED+impl: `reingreso_verificar` desde `candidate.reingreso` → terminal: remueve `bot_activo` + `requires_human`
- [x] 2.6 RED+impl: `falta_ciudad` sin `candidate.city`; `falta_experiencia` sin ninguna señal de experiencia (unidad/años/no-objetivo/ausencia); se retiran al completarse
- [x] 2.7 Tests de interacción con `perfil_listo` (objetivo+listo OK; no-objetivo / aclaración / reingreso NO permiten `perfil_listo`)
...
- [x] 2.8 Verificar que solo se emiten labels del catálogo oficial (sin deprecadas `cecati`/`escuelita`) — extender el guard existente si hace falta
...
- [x] 3a.4 Verificar que el stage pasa a HUMAN_REVIEW_REQUIRED y que no se encima pregunta de funnel tras el acuse

## 4. Wiring del contexto del worker

- [x] 4.1 Confirmar/ajustar que `app/tasks_chatwoot.py` pasa los nuevos facts a `calculate_candidate_labels` (sin cambio de firma)
- [x] 4.2 Corregir DEUDA copy `app/persona_config.py` "más de 6 meses de vigencia" → ">3 meses" (alinear con regla vigente)
