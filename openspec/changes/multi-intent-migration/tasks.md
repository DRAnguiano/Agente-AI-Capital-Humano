...
- [ ] 10b.17 **Follow-up solo por canal productivo Chatwoot** — contrato documentado en `openspec/specs/followup-scheduler/spec.md`: Chatwoot centraliza el canal operativo (WhatsApp final; webchat u otros inboxes futuros via Chatwoot). `telegram_demo` es laboratorio y solo puede generar follow-up con flag explicito (`ENABLE_DEMO_FOLLOWUP=true`). Bloquear prefijos/canales `test_*`, `debug_*`, `shadow_test*`, `test_faq*`, `test_verify*`. Implementacion pendiente en `app/followup/scheduler.py` y/o `v_temperatura_leads`, con tests rojos de elegibilidad por canal.
