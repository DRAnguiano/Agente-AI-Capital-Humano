...
- [ ] 10b.16g **Fase 2C.1** — alinear `funnel_state_planner.CORE_FIELDS` a 6-núcleo gate (sacar `availability_to_attend` del gate; tratarla como paso post-perfil) + actualizar tests 2B.1. NO wiring.
...
- [ ] 10b.16d Límite explícito 2B.1: `license.type` ≠ `license.status` (vigencia); `medical.apto_status` ≠ apto vigente. NO inferir vigencia si el fact no existe.
